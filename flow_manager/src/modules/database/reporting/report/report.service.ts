...
export class ReportService extends BaseService<Report> {
  ...
  private subdomainTableHeader = `
|IP Address|Ports|Identified Services|
|----------|-----|-------------------|\n`;
  private programTableHeader = `
...
|-----------|----------|-------------------|\n`;
   ...
   */
  public async addNewDomains(
    programName: string,
    domains: string[],
  ): Promise<void> {
    const report: Report = await this.getCurrentReport();
    if (report.programs) {
      let program: Program;
  ...
  private createDomainTreeWithIps(
    program: Program,
    domainName: string,
    ips: string[],
  ): void {
    ...
    if (report.programs) {
      report.programs.forEach((p) => {
        ...
        if (p.name === programName) {
          ...
          if (domain) {
            ...
            // The program was found, but it contained no domain tree or our domain did not exist
            this.createDomainTreeWithIps(program, domainName, ips);
          }
        }
      });
      ...
      if (!program) {
        report.programs.push(
          this.createProgramWithIps(programName, domainName, ips),
        );
      }
    } else {
      // The programs array did not exist in the report, so we create everything
      report.programs = [];
      report.programs.push(
        this.createProgramWithIps(programName, domainName, ips),
      );
...
Information in this report was unknown to Stalker before that date.\n\n`;
  ...
  private createMarkdownStringsFromTree(
    domain: Domain,
    currentParents = '',
  ): void {
    /////////////////////////////////////////////////
    ...
    // |IP|Ports|Services|
    this.programMarkdown += currentParents
      ? `|${domain.name}.${currentParents}|`
      : `|${domain.name}|`;
    this.programSubdomainsMarkdown += currentParents
      ? `### ${domain.name}.${currentParents}\n`
      : `### ${domain.name}\n`;
    ...
    if (domain.subdomains) {
      domain.subdomains.forEach((subdomain) => {
        this.createMarkdownStringsFromTree(
          subdomain,
          `${domain.name}.${currentParents}`,
        );
      });
