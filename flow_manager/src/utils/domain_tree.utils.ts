...
export function domainNameToReversedStringArray(domainName: string) {
  ...
  const splitDomains = domainName.split('.');
  // ["sub2", "sub1", "example", "com"]
  splitDomains[splitDomains.length - 2] =
    splitDomains[splitDomains.length - 2] +
    '.' +
    splitDomains[splitDomains.length - 1];
...
/**
  ...
  */
function recursiveExpandDomainTree(
  domains: Domain[],
  subdomains: string[],
  newDomains: string[],
  currentIndex = 0,
  ...
  recursiveExpandDomainTree(
    domains[domains.length - 1].subdomains,
    subdomains,
    newDomains,
    currentIndex + 1,
  );
...
/**
  ...
  */
function recursiveGrowDomainTree(
  domains: Domain[],
  subdomains: string[],
  newDomains: string[],
  currentIndex = 0,
...
/**
  ...
  */
export function growDomainTree(
  program: Program,
  fullDomainName: string,
): string[] {
  const reversedDomainNameArray =
    domainNameToReversedStringArray(fullDomainName);
  const newDomains = [];
  if (program.domains) {
    recursiveGrowDomainTree(
      program.domains,
      reversedDomainNameArray,
      newDomains,
    );
  } else {
    program.domains = [];
...
function recursiveFindDomainObject(
  domain: Domain,
  subdomains: string[],
  currentIndex = 0,
): Domain {
  ...
  domain.subdomains.forEach((subdomain) => {
    if (subdomain.name === subdomains[currentIndex]) {
      foundDomain = recursiveFindDomainObject(
        subdomain,
        subdomains,
        currentIndex + 1,
      );
...
/**
  ...
  */
export function findDomainObject(
  program: Program,
  fullDomainName: string,
): Domain {
  const reversedStringArray = domainNameToReversedStringArray(fullDomainName);
  let foundDomain: Domain = null;
 ...
 */
export function doForEveryLeaf(
  domain: Domain,
  callback: Function,
  parents = '',
): void {
  ...
  if (parents) {
    domain.subdomains.forEach((d) =>
      doForEveryLeaf(d, callback, `${domain.name}.${parents}`),
    );
  } else {
