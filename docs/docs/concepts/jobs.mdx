...
Amass also uses a the `datasources.yaml` file, used to store the API keys of several possible data sources. To use this feature in Red Kite, you can dynamically provide the `datasources.yaml` values as parameters from [Red Kite secrets](./secrets) by following the proper syntax.
