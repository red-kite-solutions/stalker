...
openssl x509 -sha256 -req -days 365 -in client1.csr -CA mongo_ca.pem -CAkey mongo_ca_private.pem -CAcreateserial -out ./flow_manager/client-signed.crt
openssl x509 -sha256 -req -days 365 -in client2.csr -CA mongo_ca.pem -CAkey mongo_ca_private.pem -CAcreateserial -out ./cron_service/client-signed.crt
...
cp mongo_ca.pem ./flow_manager/ca.pem
cp mongo_ca.pem ./cron_service/ca.pem
...
echo "  FM_MONGO_KEY_PASSWORD: $pass1" >> devspace.prod.yaml
echo "  CRON_SERVICE_MONGO_KEY_PASSWORD: $pass2" >> devspace.prod.yaml
